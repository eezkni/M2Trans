...
import time
from utils import ndarray2tensor, Gaussian_noise_layer, Cutout
import cv2
...
class US1K(data.Dataset):
    def __init__(
        self, HR_folder, LR_folder, CACHE_folder, 
        train=True, augment=True, scale=2, colors=1, 
        patch_size=96, repeat=168, add_noise=False, cutout=False
    ):
        super(US1K, self).__init__()
        ...
        if self.train and self.cutout:
            self.CutOut = Cutout(n_holes=3, length=3)
        ...
        hr_dir = os.path.join(self.cache_dir, 'us1k_hr', 'ycbcr' if self.colors==1 else 'rgb')
        lr_dir = os.path.join(self.cache_dir, 'us1k_lr_x{}'.format(self.scale), 'ycbcr' if self.colors==1 else 'rgb')
        ...
        if len(glob.glob(os.path.join(hr_dir, "*.npy"))) != len(self.hr_filenames):
            for i in range(LEN):
                ...
                # hr_image = cv2.cvtColor(cv2.imread(self.hr_filenames[i]), cv2.COLOR_BGR2RGB)
                if self.colors == 1:
        ...
        if len(glob.glob(os.path.join(lr_dir, "*.npy"))) != len(self.lr_filenames):
            for i in range(LEN):
                ...
                # lr_image = cv2.cvtColor(cv2.imread(self.lr_filenames[i]), cv2.COLOR_BGR2RGB)
                if self.colors == 1:
    ...
    def __getitem__(self, idx):
        ...
        # hr, lr = (np.load(self.hr_npy_names[idx]).astype(np.float32))/255., (np.load(self.lr_npy_names[idx]).astype(np.float32))/255.
        # print('----------', lr.dtype)
        if self.train:
            train_lr_patch, train_hr_patch = crop_patch(lr, hr, self.patch_size, self.scale, True)

            # if random.random() > 0.5 and self.add_noise:
            #     train_lr_patch, train_hr_patch = crop_patch(lr, hr, self.patch_size, self.scale, True)
            #     train_lr_patch = Gaussian_noise_layer(train_lr_patch, torch.from_numpy(np.random.uniform(-0.01, 0.01, 1).astype("float32")))
            # else:
            #     train_lr_patch, train_hr_patch = crop_patch(lr, hr, self.patch_size, self.scale, True)
            
            # if random.random() > 0.5 and self.cutout:
            #     train_lr_patch, train_hr_patch = crop_patch(lr, hr, self.patch_size, self.scale, True)
            #     for i in range(np.random.randint(1, 5)):
            #         train_lr_patch = self.CutOut(train_lr_patch)
            # else:
            #     train_lr_patch, train_hr_patch = crop_patch(lr, hr, self.patch_size, self.scale, True)
...
if __name__ == '__main__':
    ...
    HR_folder = '/SR_datasets/US1K/US1K_train_HR'
    LR_folder = '/SR_datasets/US1K/US1K_train_LR_bicubic'
    argment   = True
    us1k = US1K(HR_folder, LR_folder, augment=True, scale=2, colors=3, patch_size=384, repeat=168, store_in_ram=True)

    print("numner of sample: {}".format(len(us1k)))
    start = time.time()
    for idx in range(10):
        tlr, thr, vlr, vhr = us1k[idx]
